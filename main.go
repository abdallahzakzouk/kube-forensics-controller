...
func main() {
	...
	var exporter controllers.Exporter
	if s3Bucket != "" {
		setupLog.Info("Initializing S3 Exporter", "bucket", s3Bucket, "region", s3Region)
		s3Exp, err := controllers.NewS3Exporter(context.TODO(), s3Bucket, s3Region)
		if err != nil {
			setupLog.Error(err, "Failed to initialize S3 Exporter")
			os.Exit(1)
		}
		exporter = s3Exp
	} else {
		exporter = &controllers.NoOpExporter{}
	}
